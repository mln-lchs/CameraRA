...
public class CameraFragment extends Fragment
    ...
    private static final int MAX_PREVIEW_WIDTH = 1920;  // Max granted by API : 1920
    private static final int MAX_PREVIEW_HEIGHT = 1080; // Max : 1080
    ...
    private String mCameraId;   // Id of current camera device
    ...
    private File mFile; // Output file for picture

    static {
        System.loadLibrary("native-lib");
    }

    public native float[] testUbleam(int[] jia, int x, int y);
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            cnt++;
            mBackgroundHandler.post(new ImageProcesser(image, mOverlayView,mTextView,mImageView,cnt,CameraFragment.this));
    ...
    private class ImageProcesser implements Runnable {
        ...
        ImageProcesser(Image image, OverlayView view, TextView txt, ImageView imgView, int cnt, Fragment frag) {
            mImage = image;
            mView = view;
            mTxtView = txt;
            mImgView = imgView;
        ...
        public void run() {
            mFragment.getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    mImage.close();
                    //mTxtView.setText("width : " + w + " height : " + h);
                    int p;
                    ...
                    float[] ellipse = testUbleam(pixels, h, w);

                    float x0 = ellipse[1];
                    float y0 = ellipse[0];
                    float a = ellipse[2];
                    float b = ellipse[3];
                    float theta = ellipse[4];

                    mView.setEllipse(x0,y0,a,b,theta);
                    mView.setRect(rect);
                    mView.invalidate();
                }
            });
    ...
    private int mState = STATE_PREVIEW; // Current state of camera state for taking pictures
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                mImageReader = ImageReader.newInstance(largest.getWidth() / 16, largest.getHeight() / 16, ImageFormat.YUV_420_888, 30);
                mTextView.setText("width : " + largest.getWidth() / 16 + " height : " + largest.getHeight() / 16);
